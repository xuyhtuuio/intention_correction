...
class RAGService:
    ...
    def call_llm(self, prompt: str, temperature: float = 0.7, enable_thinking: str={"enable_thinking": False}) -> str:
        """
        调用大模型API
    ...
    def rag_query(self, query: str, top_k: int = 10, temperature: float = 0.7, enable_thinking: str={"enable_thinking": False}) -> str:
        """
        执行RAG查询：召回 -> 重排序 -> 生成回答
...
【输出要求】
1. 格式：严格遵循JSON结构
- 单意图: {"intent": "意图编码", "slots": {"槽位类型": "槽位值"}, "query": "用户查询的原始问题"}
- 多意图: {"intent": "50", "slots": [{"intent": "子意图编码", "slots": {...}}, ...], "query": "用户查询的原始问题"}
...
31 - 资产基础检索
功能：基于名称/ID/表英文名/表中文名/字段/域/类型/租户/排行/核心数据项/归属数据域/归属数据层/资产开放范围等条件查找资产
核心槽位：AssetName, AssetId, TableEnglishName, TableChineseName, AssetType, BusinessDomain, FilterCondition, AssetRanking, OwnerTenant, FieldName, CoreDataItem, DataDomain, DataLayer, AssetOpenScope
...
50 - 多意图并发查询
功能：用户一句话包含多个独立的查询意图(通过逗号/句号/"以及"/"还有"/"另外"等分隔符分隔)
输出格式：{"intent": "50", "slots": [{"intent": "31", "slots": {...}}, {"intent": "32", "slots": {...}}], "query": "原始查询"}
核心槽位：slots是数组，每个元素包含独立的intent和slots
触发条件：
  - 用户查询包含明确的分隔符(逗号/句号/"以及"/"还有"/"另外")
  - 拆分后的每个子查询对应不同的意图或不同的操作类型
注意：
  - 如果是明确的对比需求(包含"对比"/"比较"关键词)，使用Intent 37
  - 至少包含2个子意图，最多支持5个子意图
  - 每个子意图必须是31-40中的一个
...
AssetId：资产ID（纯数字，如"1169315655200010241", "1212073809486540801"）
TableEnglishName：表英文名（大写字母+下划线，如"ESD_D_CUS_HOME_DEVICE_B", "SRC_FJ_D_DSJ_5G_VIP_CHECK"）
TableChineseName：表中文名（中文字符，可能以"表"结尾，如"客户星级数据月全量加工表", "CBSS通服核销回收明细表"）
...
BusinessZone：业务专区（公众智慧运营/一线赋能专区/用来描述场景）
...
2. AssetId必须是纯数字，长度通常在13-19位之间
3. TableEnglishName必须是大写字母+下划线组成，如ESD_*、SRC_*等模式
4. TableChineseName可能与AssetName相同，或者是以"表"结尾的中文名称
5. MetadataItem需要识别同义词并规范化
6. 多个同类型槽位使用数组：{"AssetName": ["资产1", "资产2"]}
7. 无法提取的槽位直接省略，不返回null
8. intent必须是31-40或50之一，无法识别时返回"40"
9. 单意图时slots是对象格式{"SlotType": "value"}；多意图时slots是数组格式[{"intent": "31", "slots": {...}}, ...]
10. 输出纯JSON，不包含任何解释或说明
11. 归属数据层识别为ESD、DM、SRC等
12. query是用户查询的原始问题
...
判断流程：
Step 1: 是否包含"对比"/"比较"/"差异"等关键词?
      Yes → 返回Intent 37 (资产复合对比)
      No → 继续Step 2

Step 2: 是否包含分隔符(逗号/句号/分号/"以及"/"还有"/"另外"/"同时")?
      No → 单意图处理(Intent 31-40)
      Yes → 继续Step 3
...
触发Intent 50的典型场景：
"查询资产A的详情，以及资产B的负责人" (不同意图: 31 + 32)
"M域的标签和O域的指标" (相同意图但不同业务域: 31 + 31)
"订阅资产A，收藏资产B" (不同操作类型: 35 + 35)
"资产A的血缘关系，另外查询资产B的质量评分" (不同意图: 34 + 33)

不触发Intent 50的场景：
"对比资产A和资产B的业务口径" → Intent 37 (明确对比需求)
"M域的标签" → Intent 31 (单一查询)
...
{"intent": "31", "slots": {"AssetId": "1169315655200010241"}, "query":"查询资产ID 1169315655200010241"}
{"intent": "31", "slots": {"TableEnglishName": "ESD_D_CUS_HOME_DEVICE_B"}, "query":"查询表英文名为ESD_D_CUS_HOME_DEVICE_B的资产"}
{"intent": "31", "slots": {"TableChineseName": "客户星级数据月全量加工表"}, "query":"查询客户星级数据月全量加工表"}
...
{"intent": "37", "slots": {"AssetName": ["资产A", "资产B"], "MetadataItem": "业务口径"}, "query": "对比资产A和资产B的业务口径"}
{"intent": "38", "slots": {}, "query": "平台有什么功能？"}
{"intent": "38", "slots": {}, "query": "业务口径和技术口径的区别"}
{"intent": "40", "slots": {}, "query": "今天天气怎么样？"}

【Intent 50 多意图示例】
{"intent": "50", "slots": [{"intent": "31", "slots": {"AssetName": "资产A"}}, {"intent": "31", "slots": {"AssetName": "资产B"}}], "query": "我要查询资产A和资产B的资产"}
{"intent": "50", "slots": [{"intent": "31", "slots": {"AssetName": "[在线公司]宽带提质速率(月)"}}, {"intent": "32", "slots": {"AssetName": "[客户视图]用户活跃度(日)", "MetadataItem": "负责人"}}], "query": "查询宽带提质速率(月)的详情，以及用户活跃度(日)的负责人"}
{"intent": "50", "slots": [{"intent": "31", "slots": {"BusinessDomain": "M域", "AssetType": "标签"}}, {"intent": "31", "slots": {"BusinessDomain": "O域", "AssetType": "指标"}}], "query": "M域的标签和O域的指标"}
{"intent": "50", "slots": [{"intent": "34", "slots": {"AssetName": "HR系统", "LineageDirection": "上游"}}, {"intent": "33", "slots": {"AssetName": "薪资系统"}}], "query": "查询HR系统的上游依赖，另外查询薪资系统的质量评分"}
{"intent": "50", "slots": [{"intent": "31", "slots": {"AssetType": "标签", "FilterCondition": "五星"}}, {"intent": "36", "slots": {"BusinessZone": "公众智慧运营"}}], "query": "查询五星标签，同时推荐公众智慧运营场景的资产"}
"""

        if not retrieved_docs:
            self.logger.warning("未召回到相关文档，直接生成回答")
            # 使用简单提示词生成回答
            simple_prompt = single_query + f"""
            请根据以下参考信息回答用户的问题：
            ...
            simple_response = self.call_llm(simple_prompt, temperature)
            simple_ans = {
                "result": {
